...
public:
    ...
    // this is not a good habbit that use char* rather than const char*
    double computeNetPhrase(const char* lword, vector<string> words, neuron *curHiddenNeu, neuron *newHiddenNeu);
    double computeNetPhrase(const char* lword, vector<string> words, neuron *curHiddenNeu, neuron *newHiddenNeu, vector<double> &probs);
    int getHiddenSize() { return layer1_size; }
